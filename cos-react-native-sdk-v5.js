...
var request = function (params, callback) {
    ...
    if (filePath) {
        ...
        cosFormData.append('key', fileKey);
        cosFormData.append('success_action_status', 200);
        let headerKeys = [
            'Cache-Control',
            'Content-Type',
            'Content-Disposition',
        ...
        for (let i in params.headers) {
            if (params.headers.hasOwnProperty(i) && (i.indexOf('x-cos-meta-') > -1 || headerKeys.indexOf(i) > -1)) {
                ...
                cosFormData.append('x-cos-security-token', params.headers[i]);
            }
        }
        cosFormData.append('Signature', headers.Authorization);
        if (headers['x-cos-acl']) {
            cosFormData.append('acl', headers['x-cos-acl']);
        }
        if (!headers['Content-Type']) {
            cosFormData.append('Content-Type', '');
        }
        ...
        let qsStr = params.qs && obj2str(params.qs) || '';
        if (qsStr) {
    ...
    return requestTask;
        ...
        function dataURLtoFile(base64String) {//将base64转换为文件
